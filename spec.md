...
理由:
- 単月選択ではFY所属が表示データに影響しない（年度見出しは装飾に留まる）。
- デフォルト1月＝大多数がFY=暦年のため、グルーピングはほぼ無情報。
- 直近月スキャン（先月/今月）が最頻ユースケースで、フラット降順が最速。見出しは縦に伸びて逆効果。

将来、非1月会計年度（`budget_start_month ≠ 1`）の企業で年度跨ぎの混乱が顕在化した場合のみ、見出し方式ではなく**軽量なFY境界区切り線／`(FY2025)` サフィックス**（`≠1` のときだけ表示）を検討する。なお年間予実の**範囲ピッカー**は別物で、年度跨ぎ選択ニーズが出れば「FY2025を選択」等の**年度プリセット**が適切（単月ピッカーとは設計を分ける）。
