...
### 月セレクタの年度グルーピング（フェーズ2 / 任意）

月次集計・進捗分析・感度分析の月セレクタは、実装では**YYYYMM絶対値の降順フラットリスト**（過去3会計年度〜当月）。仕様モックで提案した `【2026年度】…|【2025年度】…` の年度グルーピングは**フェーズ2の任意項目**として保留（単月選択ならフラット降順で実用上十分）。

### 残論点

- **デフォルト月は `1`（1月）で確定**（下記「データモデル設計」参照）。
- 月セレクタの年度グルーピングを実施するか（フェーズ2判断）。
